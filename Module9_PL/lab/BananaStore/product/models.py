...
def fetch_one_product_process(_id):
    return Product.objects.get(id=_id)

def edit_process(_id, _name, _price, _nutrition, _origin, _product_image):
    Product.objects.filter(id=_id).update(name=_name, price=_price,
                nutrition=_nutrition,origin=_origin,product_image=_product_image)
